...
작성일: 2026-09-01 · 구현 현황 갱신: 2026-09-14

상태: Direct2D 타일 합성 및 제한형 PDF 벡터·글자 GPU 래스터화 적용

### 현재 단계: 1.32.2 / ABI v19
...
- 디스크 장면 형식 버전은 5이며 앱 버전 대신 형식 버전·네이티브 ABI·PyMuPDF 버전과 문서 해시·쪽·품질 단계·실험 옵션으로 구분한다. UI만 바뀐 릴리스는 캐시를 유지하며 추출 의미나 저장 필드가 바뀌면 형식 버전을 올린다. 기존 형식은 한 번 다시 준비해야 한다.
