...
작성일: 2026-09-01 · 구현 현황 갱신: 2026-09-11

상태: Direct2D 타일 합성 및 제한형 PDF 벡터·글자 GPU 래스터화 적용

### 현재 단계: 1.31.0 / ABI v19

- 1.31.0은 장면 추출 작업 안에서만 개체 경계와 동일 색상표의 변환 결과를 재사용한다. 경계는 개체 identity와 강한 참조로 구분하고, 색상표는 색 공간 identity·전체 샘플·불투명도·색 변환 매개변수로 구분한다. 작업이 끝나거나 실패하면 해제하며 문서 사이에서 공유하지 않는다.
- 알파 채널이 없는 회색조·CMYK 이미지 픽셀은 소유한 복사본을 먼저 기존 표시 해상도로 축소하고 RGB로 변환한다. 알파 채널이 있는 픽셀과 이미지 마스크의 처리 순서는 유지하며 확대 시에는 원본에서 다시 준비한다. 비선형 색 변환 순서에 따른 작은 경계·색 차이를 허용하는 GPU 표시 최적화이며 CPU 표시·저장·인쇄의 원본은 변경하지 않는다.
- 디스크 장면 형식 버전은 4이며 앱 버전 대신 형식 버전·네이티브 ABI·PyMuPDF 버전과 문서 해시·쪽·품질 단계·실험 옵션으로 구분한다. UI만 바뀐 릴리스는 캐시를 유지하며 추출 의미나 저장 필드가 바뀌면 형식 버전을 올린다. 기존 형식은 한 번 다시 준비해야 한다.
- 확대 후 갱신은 이미지 콜백의 종류·순번과 CPU island의 원래 영역을 기록한 장면에서 벡터·글자·그라데이션 추출을 생략한다. 마스크가 벡터 clip으로 바뀌는 등 구조가 달라지거나 타일 패턴·출처 미확인 래스터가 있으면 전체 추출로 대체한다. 벡터 항목은 재사용하고 이미지·마스크 픽셀과 작은 CPU island만 요청 배율로 다시 준비한다.
- 자동·GPU 우선 모드 모두 별도 프로세스에서 확대 후 갱신하며, 기존 픽셀을 뺀 장면 정보와 한 쪽 snapshot을 전달한다. 완료 시 문서 세대·현재 배율을 확인해 교체한다. 이 경로는 Python 벡터 재추출을 생략하지만 교체 장면의 네이티브 자원 구성은 기존 경로를 따른다.
...
Direct2D는 PDF 파일을 해석하는 엔진이 아니다. PyMuPDF가 CPU에서 PDF 명령을 해석하고 원래 glyph 윤곽과 이미지를 제공하며, Direct2D는 지원되는 장면을 래스터화·합성한다. 기존 PDF 글자를 DirectWrite로 다시 조판하지 않는다. 지원 범위와 혼합 조합의 제한은 위 현재 단계에 따른다. CPU island로 대체할 수 없는 미지원 명령이 있거나 이미지 장면 데이터가 64 MiB를 넘으면 페이지 전체를 기존 CPU 타일 경로로 표시한다.
...
상태: 진행 중. GPU 래스터화 지원 범위 확장과 별도로 성능·품질을 계속 검증한다.

### 1.30.1 캐시·초기 준비 개선

- 일반 이미지의 디스크 캐시는 원본 참조로 저장하며 CPU 합성 비트맵은 재사용한다. 용량은 끄기/50/100/250/500MB로 선택한다.
- RGBA→BGRA의 불투명 경로는 채널 일괄 처리하고 반투명 경로는 기존 정수 알파 계산을 유지한다. 동일 픽셀 객체는 GPU bitmap 한 개를 공유한다.
- 장면 메모리 예산은 이미지뿐 아니라 경로·글리프·명령 객체를 포함한다. 현재 표시하는 단일 장면을 제외한 오래된 장면을 회수한다. 실제 VRAM 자동 예산 조절은 아직 구현하지 않았다.
- 자동 모드의 확대 이미지 재준비는 독립 프로세스로 실행하며 실패한 동일 세대·배율의 반복 시도를 막는다.
...
- [x] **리더 확대 배율 표시:** 1.29.6에서 편집 입력칸의 번역 재적용을 방지해 배율 표시 복원 오류를 수정하고 DPI 기준 배율과 독립 실행 HiDPI 설정을 적용했다.
...
- [ ] **장면 메모리 상한:** 디코딩 이미지·shading·반복 자원별 필요량과 누적량을 기록한다. 64 MiB 페이지 이미지 장면 제한, CPU 타일 캐시, 256 MiB 명시적 합성 버퍼 예약 및 실제 DXGI budget을 서로 구분한다. 중복 자원 공유·지연 생성·영역별 처리와 기존 자동/낮음/보통/높음 정책을 검토하며, 원인 확인 없이 상한만 올리거나 해상도를 몰래 낮추지 않는다.
- [ ] **복잡 효과 PDF 판별과 CPU island:** 작은 shading band, 투명도 그룹, knockout 그룹이 과도하게 반복되는 페이지는 GPU 장면 추출 초기에 비용을 예측하고 세션 안에서 실패 결과를 캐시한다. 1.29.0의 CPU island는 정확 island를 먼저 시도하고, 작은 영역의 겹침/중첩 그룹은 근사 island로 GPU 표시를 유지한다. 다음 단계는 근사 island의 화질 차이를 CPU 기준 화면과 비교하고 timeout 전에 문제 그룹을 더 빨리 찾아 scene 추출 비용을 줄이는 것이다.

완료 확인 시 CPU/GPU 경로, 첫 열기·재사용 지연, 최고 RAM/VRAM 사용량, 확대 화면 비교를 함께 확인한다. GPU 표시 여부만으로 성능·품질 개선 완료를 판정하지 않는다.
